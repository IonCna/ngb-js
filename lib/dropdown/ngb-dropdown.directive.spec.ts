...
describe("ngbDropdown", () => {
    ...
    it("keeps menu open when clicking disabled items with inside autoclose", () => {
        ...
        menu.triggerHandler({ type: "click", target: disabledItem[0] } as JQueryEventObject)
        scope.$digest()
        expect(menu.hasClass("show")).toBe(true)
        expect(scope.opened).toBe(true)

        menu.triggerHandler({ type: "click", target: enabledItem[0] } as JQueryEventObject)
        scope.$digest()
    ...
    it("focuses first enabled item on ArrowDown and closes on Escape", () => {
        ...
        menu.triggerHandler({ type: "keydown", key: "ArrowDown", preventDefault: () => void 0 } as JQueryEventObject)
        scope.$digest()
        expect(document.activeElement).toBe(firstItem)

        $document.triggerHandler({ type: "keydown", key: "Escape", preventDefault: () => void 0 } as JQueryEventObject)
        scope.$digest()
        expect(menu.hasClass("show")).toBe(false)
