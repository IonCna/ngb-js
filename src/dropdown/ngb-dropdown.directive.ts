import {
	type INgbDropdownAnchor,
	NgbDropdownConfig,
} from "@ngb/dropdown/ngb-dropdown-config.service";
...
export class NgbDropdown implements IController {
	...
	$onChanges(changes: IOnChangesObject): void {
		...
		if (changes.placement && !changes.placement?.isFirstChange()) {
			const nativeContainer = this._bodyContainer
				? toNativeElement(this._bodyContainer)
				: null;
			...
			const handleInlineTab = () => {
				...
				const onFocusOut = (focusEvent: JQueryEventObject) => {
					...
					const relatedTarget =
						...
						(focusEvent.originalEvent as FocusEvent | undefined)?.relatedTarget;
					if (
						!toNativeElement(this.$element).contains(
	...
	private _applyContainer(container: null | "body" = null) {
		...
		if (container === "body") {
			this._bodyContainer =
				this._bodyContainer ?? angular.element("<div></div>");
	...
	private _applyCustomDropdownClass(newClass: string, oldClass?: string) {
		const target =
			this.container === "body" ? this._bodyContainer : this.$element;
		if (!target) return;
	...
	private _validateContainer(container?: null | string) {
		...
		throw new Error(
			`[ngb-dropdown]: Unsupported container value "${container}". Use "body" or null.`,
		);
	}
	...
	private _assertAnchor() {
		...
		throw new Error(
			`[ngb-dropdown]: NgbDropdown requires an ngbDropdownToggle or ngbDropdownAnchor.`,
		);
	}
