...
type WindowOptions = Partial<Record<WindowAttribute, unknown>> &
...
export class NgbModalWindow implements IComponentController {
	...
	$postLink(): void {
		...
		const nativeDialog = toNativeElement(this.$element).querySelector(
			".modal-dialog",
		);

		if (!nativeDialog)
			throw new Error("modal-dialog element is not present in template!");
	...
	public hide(): IPromise<void> {
		...
		const transitions = this.$q
			.all([windowTransition, dialogTransition])
			.then(() => {
	...
	private _setFocus() {
		...
		if (!native.contains(document.activeElement)) {
			const autoFocusable = native.querySelector(
				"[ngbAutofocus]",
			) as HTMLElement;
	...
	private _restoreFocus() {
		...
		const validElementToFocus =
			elWithFocus instanceof HTMLElement && body.contains(elWithFocus);
		const elementToFocus: HTMLElement = validElementToFocus
			? elWithFocus
			: body;
