...
export class NgbCarousel implements IComponentController, INgbCarousel {
	...
	$onInit(): void {
		...
		this.pauseOnFocus =
			this.pauseOnFocus ?? this.$ngbCarouselConfig.pauseOnFocus;
		this.pauseOnHover =
			this.pauseOnHover ?? this.$ngbCarouselConfig.pauseOnHover;
		this.showNavigationArrows =
			this.showNavigationArrows ?? this.$ngbCarouselConfig.showNavigationArrows;
		this.showNavigationIndicators =
			this.showNavigationIndicators ??
			this.$ngbCarouselConfig.showNavigationIndicators;
		this.showNavigationArrows =
			this.showNavigationArrows ?? this.$ngbCarouselConfig.showNavigationArrows;
	...
	focus(): void {
		if (!this._container)
			throw new Error("[ngb-carousel]: container do not exist");
	...
	prev(source?: NgbSlideEventSource) {
		if (!this.activeId)
			throw new Error("[ngb-carousel]: activeId is undefined");
	...
	private get _canCycle() {
		const hasNextSlide = this.wrap
			...
			: this.activeId != null &&
			this._getSlideIdxById(this.activeId) < this.slides.length - 1;
	...
	private _syncActiveSlideClass() {
		...
		for (const slide of this.slides) {
			this._getSlideElement(slide.id).toggleClass(
				"active",
				slide.id === this.activeId,
			);
		}
	}
	...
	private _getSlideElement(slideId: string) {
		...
		const el = toNativeElement(this._container).querySelector(
			`#slide-${slideId}`,
		);
