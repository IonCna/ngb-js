...
describe("NgbTimepicker", () => {
  ...
  it.each([
    null,
    undefined,
    { hour: undefined, minute: "aaa", second: 0 },
  ])("renders an invalid or empty model as blank inputs (%s)", (time) => {
  ...
  it.each([undefined, "medium"] as const)("uses regular Bootstrap sizing for %s", (size) => {
    const sizeAttribute = size ? ` size="'${size}'"` : "";
    const { root } = setup(`<ngb-timepicker ng-model="time"${sizeAttribute}></ngb-timepicker>`);
    expect(inputs(root).every((input) => !input.classList.contains("form-control-sm"))).toBe(true);
    expect(inputs(root).every((input) => !input.classList.contains("form-control-lg"))).toBe(true);
    expect(buttons(root).every((button) => !button.classList.contains("btn-sm"))).toBe(true);
    expect(buttons(root).every((button) => !button.classList.contains("btn-lg"))).toBe(true);
  });

  it("handles midnight and noon correctly in meridian mode", () => {
    const midnight = setup('<ngb-timepicker ng-model="time" meridian="true"></ngb-timepicker>', {
      time: { hour: 0, minute: 15, second: 0 },
    });
    expect(inputs(midnight.root).map(({ value }) => value)).toEqual(["12", "15"]);
    expect(buttons(midnight.root).at(-1)?.textContent?.trim()).toBe("AM");

    const noon = setup('<ngb-timepicker ng-model="time" meridian="true"></ngb-timepicker>', {
      time: { hour: 12, minute: 15, second: 0 },
    });
    expect(inputs(noon.root).map(({ value }) => value)).toEqual(["12", "15"]);
    expect(buttons(noon.root).at(-1)?.textContent?.trim()).toBe("PM");
  });

  it("falls back to configured steps when step inputs become undefined", () => {
    const { scope, root } = setup(
      '<ngb-timepicker ng-model="time" hour-step="hourStep" minute-step="minuteStep" second-step="secondStep" seconds="true"></ngb-timepicker>',
      { time: { hour: 1, minute: 2, second: 3 } },
    );
    const spinnerButtons = buttons(root);
    click(spinnerButtons[0]);
    click(spinnerButtons[2]);
    click(spinnerButtons[4]);
    expect(scope.time).toEqual({ hour: 2, minute: 3, second: 4 });
  });
