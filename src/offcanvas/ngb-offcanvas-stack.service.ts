...
import angular, { type IAugmentedJQuery, type ICompileService, type IPromise, type IQService, type IRootScopeService } from "angular";
...
export class NgbOffcanvasStack {
    ...
    private _attachPanelComponent(container: IAugmentedJQuery, content: IAugmentedJQuery): IPromise<ContentRef<NgbOffcanvasPanel>> {
    ...
    private _getContentRef<T>(content: any, activeOffcanvas: NgbActiveOffcanvas, options: NgbOffcanvasOptions) {
        ...
        const linkFn = this.$compile(`<${componentName} ${attrs} ngb-active-offcanvas="activeOffcanvas"></${componentName}>`);
