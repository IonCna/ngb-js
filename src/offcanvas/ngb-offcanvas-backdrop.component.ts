...
const BACKDROP_ATTRIBUTES = ["animation", "backdropClass"] as const satisfies readonly (keyof NgbOffcanvasUpdatableOptions)[];
...
export class NgbOffcanvasBackdrop implements IComponentController {
    ...
    $postLink(): void {
        ngbRunTransition(this.digestService, this.$element, ngbOffcanvasFadeInTransition, {
            animation: this.animation ?? true, runningTransition: 'continue'
        })
