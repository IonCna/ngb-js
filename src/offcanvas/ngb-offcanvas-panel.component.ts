...
import { ngbOffcanvasPanelHideTransition, ngbOffcanvasPanelShowTransition } from "@ngb/offcanvas/ngb-offcanvas-panel-transition";
...
const PANEL_ATTRIBUTES = ["animation", "ariaLabelledBy", "ariaDescribedBy", "keyboard", "panelClass", "position"] as const;
...
export class NgbOffcanvasPanel implements IComponentController {
    ...
    hide(): Observable<void> {
        ...
        const offcanvasTransition = ngbRunTransition(this.$digestService, this.$element, ngbOffcanvasPanelHideTransition, context).pipe(
    ...
    private _show(): void {
        const context: NgbTransitionOptions<unknown> = { animation: Boolean(this.animation), runningTransition: "continue" };

        const offcanvasTransition = ngbRunTransition(this.$digestService, this.$element, ngbOffcanvasPanelShowTransition, context).pipe(
            defaultIfEmpty(undefined),
        );
