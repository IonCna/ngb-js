...
function measureCollapsingElementDimensionPx(
	...
	const dimensionSize =
		toNativeElement(element).getBoundingClientRect()[dimension] + "px";
...
export function ngbCollapsingTransition(
	element: IAugmentedJQuery,
	animation: boolean,
	...
	if (!context.maxSize) {
		const maxSize = measureCollapsingElementDimensionPx(
			element,
			context.dimension,
		);
	...
	if (!context.maxSize)
		throw new Error("[ngb-transition]: context.maxSize was undefined");
