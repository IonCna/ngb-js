...
export function getFocusableBoundaryElements(element: IAugmentedJQuery) {
	...
	const list = Array.from(
		el.querySelectorAll(FOCUSABLE_ELEMENTS_SELECTOR) as NodeListOf<HTMLElement>,
...
export function ngbFocusTrap(
	element: IAugmentedJQuery,
	stopFocusTrap: IPromise<void>,
	...
	const onKeydown = (event: JQueryEventObject) => {
		...
		const isFirstOrFocused =
			focusedElement === first || focusedElement === nativeElement;
