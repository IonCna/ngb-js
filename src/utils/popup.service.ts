import type { IAugmentedJQuery, ICompileService, IDocumentService, IPromise, IQService, IRootScopeService, IScope, ITimeoutService, ITranscludeFunction } from "angular"
import { camelToKebabCase, ngbRunTransition, toNativeElement, type NgbTransitionStartFn } from "."
...
export interface IPopupService<T = any> {
    open(content?: string | ITranscludeFunction, context?: any, animation?: boolean): {
        windowRef: ContentRef<T>; transition$: IPromise<void>
    }
...
class PopupService<T> implements IPopupService<T> {
    ...
    open(content?: string | ITranscludeFunction, context?: any, animation = false) {
        ...
        const transition$ = nextRender.promise.then(() => ngbRunTransition(this.$q, this.$timeout, $element, (element) => {
