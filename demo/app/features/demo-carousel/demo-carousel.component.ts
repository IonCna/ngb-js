...
export class DemoCarouselComponent implements IComponentController {
    ...
    public onSlide({ current, prev, direction, source }: { current: string, prev: string, direction: string, source: string }) {
        this.lastEvent = `Sliding ${prev} -> ${current} (${direction}, ${source})`
    }

    public onSlid({ current, prev, direction, source }: { current: string, prev: string, direction: string, source: string }) {
